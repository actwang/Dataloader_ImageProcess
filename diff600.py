...
img_dir = '/home/zi29/Desktop/IMP/Week 3/assignment3/img_sets'
cursor = 0
...
for idx in range(8):
    current_name = [k.split('/')[-1].split('.')[0] for k in glob.glob(os.path.join(img_dir, '*.tif'))][cursor]
    current_dpi = int(current_name[2:])
    full_img_path = os.path.join(img_dir, current_name + '.tif')
    cursor += 1
    ref_index = 0
    ...
    if current_dpi == 600:
        ref_img[current_name] = img

    # otherwise resize it and add it to resize_imgs
    else:
        resized_img = cv2.resize(img, None, fx=600 / current_dpi, fy=600 / current_dpi,
                                 interpolation=cv2.INTER_LINEAR)
        resize_imgs[current_name] = resized_img

for name, img in resize_imgs:
    current_name = [k.split('/')[-1].split('.')[0] for k in glob.glob(os.path.join(img_dir, '*.tif'))][cursor]
    # full image path with diff in file name
    full_img_path = os.path.join(img_dir, current_name + 'diff.tif')
    # compute the Structural Similarity Index (SSIM) between the two
    # images, ensuring that the difference image is returned
    (score, diff) = compare_ssim(ref_img[current_name[0] + '_600.tif'], img, full=True)
